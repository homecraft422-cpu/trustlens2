...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Legacy fallback: older API may auto-create a session.
  ...
  return (
    <div className="w-full max-w-4xl mx-auto grid md:grid-cols-12 gap-8 items-center">
      ...
      <div className="md:col-span-7 bg-white border border-slate-200 rounded-3xl p-8 sm:p-10 shadow-xl shadow-slate-100/50">
        ...
        {success && verificationEmail && (
          <div className="mb-5 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm rounded-xl p-4 flex items-start gap-3">
            <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0 mt-0.5" />
            <div>
              <p className="font-bold text-emerald-900">Verify your email</p>
              <p className="mt-1">
                Account created. We sent a verification link to <strong>{verificationEmail}</strong>. Click it to sign in.
              </p>
              {devPreviewUrl && (
                <a
                  href={devPreviewUrl}
