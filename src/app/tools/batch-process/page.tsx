        ...
        prev.map((f, idx) =>
          idx === i
            ? {
                ...
                result: {
                  verdict: isFake ? "likely_ai_generated" : "likely_authentic",
                  aiScore,
                  confidence,
                },
                error: Math.random() > 0.1 ? null : "Processing failed",
              }
            : f
        )
      );
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 max-w-lg mx-auto">
            Analyze multiple files at once. Upload images, videos, and audio
            files for bulk verification.
          </p>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 mb-6">
            <div className="flex items-center justify-between flex-wrap gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={clearAll}
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50"
                >
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500">
                        {formatSize(batchFile.file.size)} • {type.toUpperCase()}
                      </p>
                    </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                              batchFile.result.verdict === "likely_authentic"
                          ...
                          <span className="text-xs text-slate-500">
                            {Math.round(batchFile.result.aiScore * 100)}%
                          </span>
                        </div>
                      )}
        ...
        {files.length > 0 && (
          <div className="flex gap-4">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white py-3 px-6 rounded-xl font-semibold hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isProcessing ? (
                <>
            ...
            <Link
              href="/"
              className="py-3 px-6 rounded-xl border border-slate-200 text-slate-700 font-semibold hover:bg-slate-50 transition-colors"
            >
              Back to Tools
            </Link>
          </div>
        ...
        {files.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-500 mb-6">
              Upload multiple files to analyze them in batch
            </p>
            <button
              onClick={() => inputRef.current?.click()}
